/*
  CRC computations
*/
#include "CRC.h"
// Define if slow version with no table is prefered
#define USE_LOWPROGMEM_SLOW

namespace CRC {

// CRC8
// https://stackoverflow.com/questions/51752284/how-to-calculate-crc8-in-c

#ifdef USE_LOWPROGMEM_SLOW

uint8_t crc8x_slow(const uint8_t* data, uint16_t len, uint8_t crc)  // init value
{
  uint16_t i, j;
  for (i = 0; i < len; i++) {
    crc ^= data[i];
    for (j = 0; j < 8; j++) {
      if ((crc & 0x80) != 0)
        crc = (uint8_t)((crc << 1) ^ 0x31);
      else
        crc <<= 1;
    }
  }
  return crc;
}

uint8_t crc8(const void* mem, uint16_t len, uint8_t prevCrc8) {
  return crc8x_slow((const uint8_t*)mem, len, prevCrc8);
}

#else

static unsigned uint8_t const crc8x_table[] PROGMEM = {
  
};

uint8_t crc8x_fast(const uint8_t* data, uint16_t len, uint8_t crc) {
  if (data == NULL)
    return 0xff;
  crc &= 0xff;
  while (len--) {
    int index = crc ^ *data++;
    crc = pgm_read_byte_near(crc8x_table + index);
  }
  return crc;
}

uint8_t crc8(void const* mem, uint16_t len, uint8_t prevCrc8) {
  return crc8x_fast(mem, len, prevCrc8);
}

#endif

}
